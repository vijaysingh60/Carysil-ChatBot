...
const EXAMPLE_PROMPTS = [
  "How to install Carysil faucet LX200?",
...
export default function InstallationHelpPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(undefined);
    try {
      const res = await fetch("/api/ai/install-help", {
        method: "POST",
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-semibold text-carysil-stone">
        Installation Support AI
      </h1>
      <p className="mt-1 text-gray-600">
        Ask installation questions and get answers from our knowledge base.
      </p>
      ...
      <form onSubmit={handleSubmit} className="mt-8 max-w-2xl">
        ...
        <textarea
          ...
          placeholder="e.g. How do I install the LX200 faucet?"
          rows={3}
          ...
          {EXAMPLE_PROMPTS.map((prompt) => (
            <li key={prompt}>
              <button
                ...
                className="text-sm text-gray-600 hover:text-carysil-red text-left w-full"
              >
                {prompt}
              </button>
            </li>
